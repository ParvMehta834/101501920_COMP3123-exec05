...
--- ANS: Error handling in Express is implemented using middleware functions that have four parameters (err, req, res, next).
...
--- ANS: The line app. listen(process. env. port || 8081) allows the application to use the port that's in the port environment variable, if one is defined; otherwise it uses port 8081. The methods have several advantages in production sites: 
1.Deployment Flexibility: In cloud platforms, ports are often assigned dynamically which accommodate different deployment patterns.
2.Environment Configuration: You can also set up different environments (eg. development, staging, production) to use other port numbers and thus easier swtich between each stage.
3.Platform agnostic: This approach is now independent on the hosting service as there are some services that describe PORT as an environment variable and some not.
